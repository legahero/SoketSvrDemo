      ...
      m_transmitPos(0)
{
    ...
    connect(this, SIGNAL(readyRead()), this, SLOT(doParseRequest()));//这个在应用中必须实现
...
{
    qDebug() << "QAsynTcpSocket::doDisconnected";
...
void QAsynTcpSocket::doParseRequest()
{
    qDebug() << "QAsynTcpSocket:doParseRequest,ThreadId:"<<QThread::currentThreadId()  ;

    while (this->bytesAvailable()) {

        QByteArray arr = this->readAll();
        qDebug() << "readAll:"<<arr  ;
        Q_EMIT newRequest(this,arr );

        qDebug() << "doParseRequest end,ThreadId:"<<QThread::currentThreadId()  ;
    }
}
