...
void ThreadHandle::initThreadType(ThreadType type, unsigned int max)
{
    if (!initfist)
    {
        ...
        else
        {
            ...
            //qDebug()<<"Thread(0x"<<qint32(&tmp)<<")" ;//企图将指针类型转换为无符号32位整型数，但这在64位环境下会发生兼容性问题。
            qDebug()<<"Thread(0x"<<&tmp<<")" ;
...
void ThreadHandle::initThreadSize() //建立好线程并启动，
{
    ...
    {
        tmp = new QThread;
        //qDebug()<<"Thread(0x"<<qint32(Qt::HANDLE(tmp))<<")" ;//企图将指针类型转换为无符号32位整型数，但这在64位环境下会发生兼容性问题。
        qDebug()<<"Thread(0x"<<Qt::HANDLE(tmp)<<")" ;
#ifndef Q_OS_WIN
        tmp->setEventDispatcher(new EventDispatcherLibEv());
#endif
        threadSize.insert(tmp,0);
